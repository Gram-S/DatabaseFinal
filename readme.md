This project has ended up focusing a little more on using SQL as an intermediate step for storing/querying data and having python analyze it. In the original project, every data structure is dependant on another, so it is difficult to justify implamenting a CRUD format into all of them. Instead, I only implamented a CRUD format into the "input data" section, so users can upload their data and have python analyze it to create the other data structures. It is my hope that this allows a user to understand the original project a little better. Additionally, while I do use some fairly advanced aggretition in sql, the main complexity comes from using python to analyze a sql database. 
